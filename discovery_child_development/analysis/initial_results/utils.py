...
def load_openalex_data():
    ...
    return (
        data_df
        .merge(metadata_df[['id', 'year']], how='left', on='id')
...
def load_crunchbase_data():
    cb_data_df = (
        pd.read_csv(ENRICHED_DATA_DIR / 'crunchbase_combined_labels_checked.csv')
    )    
    cb_country_codes = (
    ...
    return (
        ...
        .query("year >= 2013 and year <= 2023")
        .merge(cb_data_df[['id', 'text', 'topics']], left_on='org_id', right_on='id', how='left')
    ...
    )[['id', 'text', 'dataset', 'topics', 'year', 'country_code', 'amount', 'investment_type', 'org_id']]    
...
def adjust_by_uk_inflation(df, column='amount', reference_year=2019):
    inflation_df = (
        pd.read_csv(INPUTS_DATA_DIR / 'gdp_deflator.csv')
        .query("year >= 2013 and year <= 2023")
    )
    target_deflator = inflation_df.query("year == @reference_year")['deflator'].values[0]
    return (
        df
        .merge(inflation_df[['year', 'deflator']], on='year', how='left')
...
def get_geographical_distribution(data_exploded_df, column='id'):
    ...
    for country_code in country_codes:
        ...
        _ts_df = get_timeseries(country_df, column=column).assign(country_code = country_code)
        growth_df.append(
...
def load_topic_data():
    ...
    return (
        ...
        .sort_values(['type', 'subtype', 'topic',])
        .reset_index(drop=True)
...
def plot_quick_ts(data_df, column, value_denominator=1):
    fig = pu.ts_smooth(
        (
            data_df
            .assign(Total="Total")
            .assign(amount = lambda df: df[column]/value_denominator)
        ),
        ["Total"],
        variable= "amount",
        variable_title = "",
...
def explode_data(data_df, column="topics"):
    return (
        data_df
        .fillna({column: ","})
        .assign(topics=lambda df: df.topics.apply(lambda x: [x.strip() for x in x.split(",")]))
        .explode('topics')
        .drop_duplicates(subset=['id', 'topics'])
        .merge(TOPICS_DF, left_on='topics', right_on='topic', how='left', suffixes=('', '_'))
    )
...
def get_timeseries(data_df, groupby=['year'], column='id'):
    ...
    return (
        ...
        ts_df.assign(year = lambda df: pd.to_datetime(df.year, format='%Y')), 'year', 'Y', 2013, 2023)
...
def impute_empty_periods_all_ts(ts_df, cat_column):
    ...
    for cat in ts_df[cat_column].unique():
        dfs.append(
            au.impute_empty_periods(ts_df.query(f"{cat_column} == '{cat}'").assign(year = lambda df: pd.to_datetime(df.year, format='%Y')), 'year', 'Y', 2013, 2023)
...
def get_data_distribution(data_exploded_df, column, values, ts: bool = False):
    ...
    for value in values:
        ...
        _df = (
            ...
            .reset_index()
            .assign(**{f'{column_name}_prop': lambda df: round(df[column_name] / normalisation_value, 3)})  
        )
        if ts:
    ...
    if column == 'type':
        return df
    if column == 'subtype':
        return df.merge(TOPICS_DF[['type', 'subtype']].drop_duplicates(), on='subtype', how='left')
...
def get_data_magnitude_growth(data_exploded_df, ids, column, value):    
    ...
    tech_applications_ts = get_data_distribution(
        data_exploded_df.query('id in @ids'),
        column=column, 
        values=[value],
        ts=True
    )    
