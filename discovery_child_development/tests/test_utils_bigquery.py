...
def test_create_client_without_credentials_and_successful_download():
    ...
    with patch(
        "discovery_child_development.utils.google_utils.path.isfile", return_value=False
    ):
        with patch(
            "discovery_child_development.utils.google_utils.download_file"
        ) as mock_download:
            mock_download.return_value = None  # Successful download
            with patch(
                "discovery_child_development.utils.google_utils.Credentials.from_service_account_file"
            ) as mock_credentials:
                mock_credentials.return_value = None
                with patch(
                    "discovery_child_development.utils.google_utils.bigquery.Client.__init__"
                ) as mock_client:
...
def test_create_client_without_env_variable():
    ...
    """
    with patch.dict(
        "discovery_child_development.utils.google_utils.environ", {}, clear=True
    ):
