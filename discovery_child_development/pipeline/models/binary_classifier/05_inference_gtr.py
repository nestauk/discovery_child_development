...
python discovery_child_development/pipeline/models/binary_classifier/05_inference_gtr.py
...
if __name__ == "__main__":
    ...
    data_for_labelling = pd.read_csv(ENRICHED_DATA_DIR / "gtr_texts.csv").dropna(
        subset=["text"]
    )
    ...
    S3.upload_obj(
        predictions,
        S3_BUCKET,
        f"data/outputs/binary_classifier/gtr_texts_relevance_labelled.csv",
    )
