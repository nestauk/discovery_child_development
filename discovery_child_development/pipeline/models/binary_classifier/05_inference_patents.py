...
python discovery_child_development/pipeline/models/binary_classifier/05_inference_patents.py
...
if __name__ == "__main__":
    ...
    data_for_labelling = pd.concat(
        [patents_df[["id", "text"]]], ignore_index=True
    ).dropna(subset=["text"])
    logging.info(f"Number of patents to label: {len(data_for_labelling)}")
    ...
    S3.upload_obj(
        predictions,
        S3_BUCKET,
        f"data/outputs/binary_classifier/patents_{OUTPUT_FILENAME}.csv",
    )
