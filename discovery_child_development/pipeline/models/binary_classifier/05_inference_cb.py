...
python discovery_child_development/pipeline/models/binary_classifier/05_inference_cb.py
...
if __name__ == "__main__":
    ...
    # to do: change to loading from s3
    data_for_labelling = pd.read_csv(ENRICHED_DATA_DIR / "crunchbase_texts.csv").dropna(
        subset=["text"]
    )
    ...
    S3.upload_obj(
        predictions,
        S3_BUCKET,
        f"data/outputs/binary_classifier/crunchbase_texts_relevance_labelled.csv",
    )
